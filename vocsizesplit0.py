...
class VocEditor:
	...
	def conv(self, name, dest_dir, vggize):
		...
		if len(self.object_list) <= 0:
			log(1, name + ".xml object not found in xml")
			return
		if len(self.object_list) <= 0 or self.width <= vggize and self.height < vggsize:
			log(1, name + ".jpg image size too small")
			return 
...
if __name__ == '__main__':
	...
	for fname in flist:
		...
		if not Path("{}.jpg".format(name)).is_file() :
			log(1, "{}.jpg not found".format(name))
		elif not Path("{}.xml".format(name)).is_file():
			log(1, "{}.xml not found".format(name))
		else:
